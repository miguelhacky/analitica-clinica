...
@api_view(['GET'])
@permission_classes([permissions.IsAuthenticated])
def enfermedades_contagiosas_list(request):
    enfermedades = ContagiousDisease.objects.all().values('id', 'nombre')
    return Response(list(enfermedades))
...
def reportes_view_api(request):
    ...
    df = pd.DataFrame(list(pacientes)).fillna('')
    ...
    if tipo == 'excel':
        ...
        for sheet in wb.worksheets:
            for col_idx, col_cells in enumerate(sheet.columns, 1):
                ...
                for cell in col_cells:
                    ...
                    if cell.number_format and 'yyyy' in cell.number_format:
                        cell.number_format = 'DD/MM/YYYY'
                sheet.column_dimensions[get_column_letter(col_idx)].width = min(max(max_len + 3, 10), 45)
        wb.save(output_final)
        output_final.seek(0)
        resp = HttpResponse(output_final.read(), content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        ...
        if p_list:
            ...
            data = [headers] + [[(str(p.get(h, '')) if p.get(h) is not None else '')[:30] for h in headers] for p in p_list]
